...
SubDivMeshRenderer makeRenderer(SubDivMesh & m, bool trust_neighbours) {
	
	const auto &e = m.e;
	const auto &n = m.n;
	
	// normales por elemento (no unitarias, para que se ponderen por area del elemento) 
	std::vector<glm::vec3> enorms;
	enorms.reserve(e.size());
	for(const Elemento &ei : e) {
		glm::vec3 normal = glm::cross(n[ei[2]].p-n[ei[0]].p,n[ei[1]].p-n[ei[0]].p);
		if (ei.nv==4) {
			glm::vec3 normal2 = glm::cross(n[ei[3]].p-n[ei[0]].p,n[ei[2]].p-n[ei[0]].p);
			normal = (normal+normal2)/2.f;
		}
	...
	for(int ie=0;ie<static_cast<int>(e.size());++ie) {
		...
		if ((not trust_neighbours) or ei.v[0]<ie) { lines.push_back(ei[0]); lines.push_back(ei[1]); }
		if ((not trust_neighbours) or ei.v[1]<ie) { lines.push_back(ei[1]); lines.push_back(ei[2]); }
		if ((not trust_neighbours) or ei.v[2]<ie) { lines.push_back(ei[2]); lines.push_back(ei[3]); }
		if (ei.nv==3) continue;
		tris.push_back(ei[0]); tris.push_back(ei[2]); tris.push_back(ei[3]);
		if ((not trust_neighbours) or ei.v[3]<ie) { lines.push_back(ei[3]); lines.push_back(ei[0]); }
	}
