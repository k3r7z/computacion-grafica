...
void SubDivMesh::verificarIntegridad() const {
	...
	// ver para cada nodo de un elemento, que el nodo tenga la ref al elemento en su e
	for (int ie=0; ie<esz; ++ie) { 
		const Elemento &ei = e[ie];
		for (int in : ei) {
			cg_assert(in>=0 and in<nsz,"El elemento "<<ie<<" tiene un indice de vertice no valido: "<<in);
			auto &enj = n[in].e;
	...
	for (int in=0; in<nsz; ++in) {
		const Nodo &ni = n[in];
		for (int ie : ni.e) {
			cg_assert(ie>=0 and ie<esz,"El nodo "<<in<<" tiene un indice de elemento no valido: "<<ie);
