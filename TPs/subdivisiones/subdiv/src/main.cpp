...
std::vector<Arista>& getEdgesFromElement(const Elemento & element){
    std::vector edges();
    for( int nodeIndex : element )
	edges.push_back( Arista( nodeIndex, nodeIndex + 1) );
    return edges;
}


void addNewEdgePoints() {
    // Iterate elements of the mesh using an index
    std::vector<Nodo> & nodes = mesh.n;
    std::vector<Elemento> & elements = mesh.e;

    for(Elemento & element : elements ) {
	const Node & centerPoint = centerPoints.find(


	std::vector<Arista> edges = getEdgesFromElement(element);
	...
	for(int i = 0; i < element.nv; ++i){
	    Nodo & edgeStartPoint = nodes[ element.n[i] ];
	    Nodo & edgeEndPoint = nodes[ element.n[i+1] ];

	    double average_x = edgeEndPoint.p.x - edgeStartPoint.p.x;
	    double average_y = edgeEndPoint.p.y - edgeStartPoint.p.y;
	    double average_z = edgeEndPoint.p.z - edgeStartPoint.p.z;

	    // Search for the neighbor that shares the same edge, i.e., the same nodes

	    // NOTE: what if the element is a triangle? should it also have 4 neighbors?
	    Element * neighbor = null;
	    for(int j = 0; j < 4; ++j){
		neighbor = &elements[ element.v[j] ];
		if(neigh
		

	    
	    
	    Nodo edgeMidPoint( glm::vect3(average_x, average_y, average_z));
...
void subdivide(SubDivMesh &mesh) {
    // This map saves the relationship between an element and its new center point
    // This is done by saving the element's index and the node's index respectively
    std::map<int, int> centerPoints;
	...
	const Elemento element = elements[elementIndex];

	// Make the average of the nodes 
	double average_x, average_y, average_z;
	for(int nodeIndex = 0 ; nodeIndex < element.nv ; ++nodeIndex){
	    Nodo node = nodes[ element.n[nodeIndex] ];
	    average_x += node.p.x;
	    average_y += node.p.y;
	    average_z += node.p.z;
	}

	// Make a new node with the calculated average values
	Nodo node( glm::vec3(average_x, average_y, average_z) );

	centerPoints[elementIndex] = node_index;
    ...
    // At this point, every element of the mesh has its own new center point
    for(int elementIndex = 0; elementIndex < elements.size(); ++elementIndex) {
	const Elemento element = elements[elementIndex];

	// Get current element's new center point
	const Nodo & centerPoint = centerPoints.find[ elementIndex ];

	for(int nodeIndex : element.n ){
	    Node & edgeStartPoint = nodes[ element.n[nodeIndex] ];
	    Node & edgeEndPoint = nodes[ element.n[nodeIndex + 1] ];

	    double average_x, average_y, average_z;
	    
	    // Neighbor exists
	    if( element.v[ nodeIndex ] != -1 ) {
		Node & neighborCenterPoint = centerPoints.find( element.v[ nodeIndex ] );
		average_x = (edgeStartPoint.p.x + edgeEndPoint.p.x + centerPoint.p.x + neighborCenterPoint.p.x) / 4;
		average_y = (edgeStartPoint.p.y + edgeEndPoint.p.y + centerPoint.p.y + neighborCenterPoint.p.y) / 4;
		average_z = (edgeStartPoint.p.z + edgeEndPoint.p.z + centerPoint.p.z + neighborCenterPoint.p.z) / 4;
		
		
	    }
	    else { // No neighbor
		average_x = (edgeStartPoint.p.x + edgeEndPoint.p.x) / 2;
		average_y = (edgeStartPoint.p.y + edgeEndPoint.p.y) / 2;
		average_z = (edgeStartPoint.p.z + edgeEndPoint.p.z) / 2;
		
	    }

	    Node edgeMidPoint( glm::vec3(average_x, average_y, average_z) );
	    nodes.push_back(edgeMidPoint);
